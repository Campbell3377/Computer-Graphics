...
// Note -- the ternary statement above is not good programming practice. Should check if r smaller than some zero tolerance 
...
// (two triangles/quad) from data
	...
	console.log("pointsArray for GPU built");
	console.log("pointsArray length = ",pointsArray.length);
	console.log("first point = ",pointsArray[0]);
	console.log("last point = ",pointsArray[pointsArray.length-1]);
	...
	console.log("ytop = ",ytop,"  bottom = ",bottom);
...
// buttons for moving viewer and changing size
    ...
    document.getElementById("Button13").onclick = function(){
		if(polygonOffset == 1) {
			...
			console.log("polygon offset OFF");
		}
		else {
			polygonOffset = 1;
			gl.depthFunc(gl.LEQUAL);
		    gl.enable(gl.POLYGON_OFFSET_FILL);
			gl.polygonOffset(1.0, 2.0);
			console.log("polygon offset ON");
		}
		};	  
	document.getElementById("Button14").onclick = function(){
		if(triMeshToggle == 1) {
			triMeshToggle = 0;
			console.log("triangle mesh not drawn");
		}
		else {
			triMeshToggle = 1;
			console.log("triangle mesh drawn for hidden surface removal");
		}
		};
...
function render()
{
    ...
    for(var i=0; i<pointsArray.length; i+=4) { 
		if(triMeshToggle == 1) {
			gl.uniform4fv(fColor, flatten(currentColor));
